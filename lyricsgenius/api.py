...
class Genius(API):
    ...
    def search_song(self, title, artist="", get_full_info=True, printconsole = True):
        ...
        if self.verbose:
            if artist:
                if printconsole: print('Searching for "{s}" by {a}...'.format(s=title, a=artist))
            else:
                if printconsole: print('Searching for "{s}"...'.format(s=title))
        ...
        if not result:
            if self.verbose:
                if printconsole: print("No results found for: '{s}'".format(s=search_term))
            return None
        ...
        if not valid:
            if self.verbose:
                if printconsole: print('Specified song does not contain lyrics. Rejecting.')
            return None
        ...
        if not lyrics:
            if self.verbose:
                if printconsole: print('Specified song does not have a valid URL with lyrics. Rejecting.')
            return None

        # Return a Song object with lyrics if we've made it this far
        song = Song(song_info, lyrics)
        if self.verbose:
            if printconsole: print('Done.')
        return song
